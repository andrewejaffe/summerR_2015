...
```{r colMeans, prompt=TRUE,comment=""}
...
```{r summary1, prompt=TRUE,comment=""}
summary(circ2)
```
...
```{r scatter1, comment="",prompt=TRUE}
data(cars)
...
```{r hist1, comment="",prompt=TRUE}
hist(circ2$daily)
```

## Density

```{r dens1, comment="",prompt=TRUE,fig.width=5,fig.height=5}
...
```{r box2, comment="",prompt=TRUE}
boxplot(daily ~ day, data=circ2)
```

## Matrix plot

```{r matplot1, comment="",prompt=TRUE}
matplot(circ2[,3:6])
```
